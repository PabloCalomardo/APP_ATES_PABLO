...
def PRA(forest_type, DEM, FOREST, radius, prob, winddir, windtol, pra_thd, sf, out_dir="PRA"):
    ...
    f = open(out_dir / "log.txt", "w+", encoding="utf-8")
    now = datetime.now()
    ...
    with rasterio.open(out_dir / 'windshelter.tif', "w", **profile) as dest:
        dest.write(data)
    ...
    # --- Save raster to path using meta data from dem.tif (i.e. projection)
    with rasterio.open(out_dir / 'PRA_continous.tif', "w", **profile) as dest:
    ...
    # --- Remove islands smaller than sf pixels
    sievefilter = sf + 1
    pra_binary_path = out_dir / 'PRA_binary.tif'

    if gdal is not None:
        Image = gdal.Open(str(pra_binary_path), 1)  # open image in read-write mode
        Band = Image.GetRasterBand(1)
        gdal.SieveFilter(
            srcBand=Band,
            maskBand=None,
            dstBand=Band,
            threshold=sievefilter,
            connectedness=8,
            callback=gdal.TermProgress_nocb,
        )
        del Image, Band  # close the datasets.
    else:
        # Fallback when GDAL python bindings are not available.
        # Remove small connected components of PRA==1 using 8-neighbour connectivity.
        from skimage.morphology import remove_small_objects

        with rasterio.open(pra_binary_path) as src:
            arr = src.read(1)
            prof = src.profile.copy()
            nodata = src.nodata

        valid = np.ones(arr.shape, dtype=bool) if nodata is None else (arr != nodata)
        binary = (arr == 1) & valid
        cleaned = remove_small_objects(binary, min_size=int(sievefilter), connectivity=2)

        out_arr = arr.copy()
        out_arr[valid] = 0
        out_arr[cleaned] = 1

        with rasterio.open(pra_binary_path, 'w', **prof) as dst:
            dst.write(out_arr.astype(prof.get('dtype', 'int16')), 1)
...
if __name__ == "__main__":
    ...
    if forest_type in ['pcc', 'stems', 'bav']:
        ...
        sf = int(sys.argv[9])
        out_dir = sys.argv[10] if len(sys.argv) >= 11 else "PRA"
        PRA(forest_type, DEM, FOREST, radius, prob, winddir, windtol, pra_thd, sf, out_dir=out_dir)
    if forest_type in ['no_forest']:
        DEM = sys.argv[2]
        ...
        sf = int(sys.argv[8])
        out_dir = sys.argv[9] if len(sys.argv) >= 10 else "PRA"
        PRA(forest_type, DEM, DEM, radius, prob, winddir, windtol, pra_thd, sf, out_dir=out_dir)
