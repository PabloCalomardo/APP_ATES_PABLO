...
def source_id_to_bitmask(source_id):
    """Convert a PRA source id (1..64) into an uint64 bitmask."""
    source_id_int = int(source_id)
    if source_id_int <= 0 or source_id_int > 64:
        return np.uint64(0)
    return np.left_shift(np.uint64(1), np.uint64(source_id_int - 1))
...
def split_release(release, header_release, pieces):
    ...
    release[release < 0] = 0
    release_binary = (release > 0).astype(np.uint8)
    summ = np.sum(release_binary) # Count number of release pixels
    ...
    for i in range(breakpoint_x, release.shape[1]):
        if len(release_list) == (pieces - 1):
            ...
            break
        if np.sum(release_binary[:, breakpoint_x:i]) < sum_per_split:
            continue
        else:
            c = np.zeros_like(release)
...
def calculation(args):
    ...
    source_id_array = np.zeros(dem.shape, dtype=np.uint64)
    fp_travelangle_array = np.zeros_like(dem)
    fp_distance_array = np.ones_like(dem) * 10001
    back_list = []
    source_limit_warning_printed = False
    ...
    while startcell_idx < len(row_list):
        ...
        start_source_id = int(release[row_idx, col_idx])
        start_source_mask = source_id_to_bitmask(start_source_id)
        if start_source_mask == 0 and (start_source_id > 64) and not source_limit_warning_printed:
            print("Warning: PRA source id > 64 found. These IDs cannot be represented in uint64 bitmask and are ignored.")
            source_limit_warning_printed = True
        ...
        for idx, cell in enumerate(cell_list):
            ...
            source_id_array[cell.rowindex, cell.colindex] = np.bitwise_or(source_id_array[cell.rowindex, cell.colindex], start_source_mask)
    ...
    return z_delta_array, flux_array, count_array, z_delta_sum, backcalc, source_id_array, fp_travelangle_array, fp_distance_array
