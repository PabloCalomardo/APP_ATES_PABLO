...
def get_pra_ids(release, release_header):
    """Extract sorted PRA IDs (>0) from release raster, excluding nodata."""
    nodata = release_header.get('noDataValue')
    valid_mask = release > 0
    if nodata is not None:
        valid_mask = np.logical_and(valid_mask, release != nodata)
    valid_values = release[valid_mask]
    return sorted({int(value) for value in valid_values if int(value) > 0})


def build_source_multiband(source_ids_mask, pra_ids):
    """Decode uint64 bitmask raster into multi-band binary raster."""
    valid_pra_ids = [pra_id for pra_id in pra_ids if 1 <= int(pra_id) <= 64]
    if len(valid_pra_ids) == 0:
        return np.zeros((0, source_ids_mask.shape[0], source_ids_mask.shape[1]), dtype=np.uint8), []

    multiband = np.zeros((len(valid_pra_ids), source_ids_mask.shape[0], source_ids_mask.shape[1]), dtype=np.uint8)
    for band_idx, pra_id in enumerate(valid_pra_ids):
        bitmask = np.left_shift(np.uint64(1), np.uint64(int(pra_id) - 1))
        multiband[band_idx] = (np.bitwise_and(source_ids_mask, bitmask) > 0).astype(np.uint8)
    return multiband, valid_pra_ids


def write_source_band_mapping(mapping_path, pra_ids):
    with open(mapping_path, 'w', encoding='utf-8') as mapping_file:
        mapping_file.write('band,pra_id\n')
        for band_idx, pra_id in enumerate(pra_ids, start=1):
            mapping_file.write('{},{}\n'.format(band_idx, pra_id))
...
class Flow_Py_EXEC():
    ...
    def calculation(self):
        ...
        self.pra_ids = get_pra_ids(release, release_header)
        logging.info('PRA IDs detected: {}'.format(self.pra_ids))
        ...
        self.source_ids = np.zeros(dem.shape, dtype=np.uint64)
        self.fp_ta = np.zeros_like(dem)
        self.fp_dis = np.ones_like(dem) * 10000
    ...
    def thread_finished(self, z_delta, flux, count_array, z_delta_sum, backcalc, source_ids, fp_ta, fp_dis):
        ...
        for i in range(len(z_delta)):
            ...
            self.source_ids = np.bitwise_or(self.source_ids, source_ids[i])
            self.fp_ta = np.maximum(self.fp_ta, fp_ta[i])
            self.fp_dis = np.minimum(self.fp_dis, fp_dis[i])
        self.output()
    ...
    def output(self):
        ...
        io.output_raster(self.ui.DEM_lineEdit.text(),
                         ...
                         self.fp_dis)
        io.output_raster(self.ui.DEM_lineEdit.text(),
                         self.directory + self.res_dir + "source_ids_bitmask{}".format(self.ui.outputBox.currentText()),
                         self.source_ids,
                         nodata=0)

        source_multiband, valid_pra_ids = build_source_multiband(self.source_ids, self.pra_ids)
        if source_multiband.shape[0] > 0:
            io.output_raster(self.ui.DEM_lineEdit.text(),
                             self.directory + self.res_dir + "source_ids_multiband.tif",
                             source_multiband,
                             nodata=0)
            write_source_band_mapping(self.directory + self.res_dir + "source_ids_multiband_bands.csv",
                                      valid_pra_ids)
        if len(self.pra_ids) > len(valid_pra_ids):
            logging.warning('PRA IDs > 64 found and omitted from bitmask/multiband output: {}'.format(
                [pra_id for pra_id in self.pra_ids if pra_id > 64]))
...
def main(args, kwargs):
    ...
    pra_ids = get_pra_ids(release, release_header)
    logging.info('PRA IDs detected: {}'.format(pra_ids))
    ...
    source_ids = np.zeros(dem.shape, dtype=np.uint64)
    ...
    if calc_bool:
        ...
        test = [[dem, header, infra, forest, release_pixel, alpha, exp, flux_threshold, max_z]
    ...
    for i in range(len(results)):
        ...
        source_id_list.append(res[5])
        fp_ta_list.append(res[6])
        fp_dis_list.append(res[7])
    ...
    for i in range(len(z_delta_list)):
        ...
        source_ids = np.bitwise_or(source_ids, source_id_list[i])
        fp_ta = np.maximum(fp_ta, fp_ta_list[i])
        fp_dis = np.minimum(fp_dis, fp_dis_list[i])
    ...
    io.output_raster(dem_path,
                     ...
                     fp_dis)
    io.output_raster(dem_path,
                     directory + res_dir + "source_ids_bitmask{}".format(output_format),
                     source_ids,
                     nodata=0)

    source_multiband, valid_pra_ids = build_source_multiband(source_ids, pra_ids)
    if source_multiband.shape[0] > 0:
        io.output_raster(dem_path,
                         directory + res_dir + "source_ids_multiband.tif",
                         source_multiband,
                         nodata=0)
        write_source_band_mapping(directory + res_dir + "source_ids_multiband_bands.csv",
                                  valid_pra_ids)
    if len(pra_ids) > len(valid_pra_ids):
        logging.warning('PRA IDs > 64 found and omitted from bitmask/multiband output: {}'.format(
            [pra_id for pra_id in pra_ids if pra_id > 64]))
